    ...
    <>
      {positions.length > 0 ? (
        <div className="grid gap-1.5 sm:grid-cols-2 lg:grid-cols-3">
          {positions.map((p) => {
            const accepted = countForPosition(applications, p.positionName, "APPROVED");
            const legacyPending = countForPosition(applications, p.positionName, "PENDING");
            return (
              <div
                key={p.positionName}
              ...
              >
                ...
                <p className="mt-1 text-[11px] leading-tight text-muted-foreground">
                  ...
                  受付済{" "}
                  <span className="font-medium tabular-nums text-foreground">{accepted}</span>
                  {legacyPending > 0 ? (
                    <>
                      <span className="mx-1 opacity-50">·</span>
                      審査中(旧){" "}
                      <span className="font-medium tabular-nums text-foreground">{legacyPending}</span>
                    </>
                  ) : null}
                </p>
              </div>
      ...
      <div className="flex flex-wrap items-center gap-x-3 gap-y-0.5 text-[11px] text-muted-foreground">
        <span>
          総数 <strong className="tabular-nums text-foreground">{applications.length}</strong>
        </span>
        <span className="hidden sm:inline opacity-40">|</span>
        {pendingTotal > 0 ? (
          <span>
            審査中(旧){" "}
            <strong className="tabular-nums text-foreground">{pendingTotal}</strong>
          </span>
        ) : null}
        <span>
          受付済 <strong className="tabular-nums text-foreground">{approvedTotal}</strong>
        </span>
      </div>
    ...
    return (
      <div className="space-y-3 border-t border-border pt-4">
        <div className="space-y-1">
          <h3 className="text-sm font-semibold">応募状況</h3>
          <p className="text-xs leading-relaxed text-muted-foreground">
            大会ページからの応募一覧です。不要な応募は削除できます。
          </p>
        </div>
        {inner}
      </div>
    ...
    <Card className="overflow-hidden">
      <CardHeader className="space-y-1 border-b border-border bg-muted/15 px-4 py-3">
        ...
        <CardDescription className="text-xs leading-relaxed">
          大会ページからの応募一覧です。ポジション別の受付数は募集人数の目安にどうぞ。
        </CardDescription>
      </CardHeader>
