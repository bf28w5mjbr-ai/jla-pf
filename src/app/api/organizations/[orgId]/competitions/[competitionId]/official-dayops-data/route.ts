...
import { loadOfficialApplicationsDayOpsData } from "@/lib/officialApplicationAdminData";
...
export async function GET(
  ...
  try {
    ...
    const data = await loadOfficialApplicationsDayOpsData(prisma, competitionId);
    return NextResponse.json(data);
  } catch (error) {
    return jsonInternalError500(
      "GET api/organizations/.../official-dayops-data",
      error
    );
  }
}
