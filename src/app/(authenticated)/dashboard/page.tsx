...
export default async function DashboardPage() {
  ...
  const officialAttendanceWeightedDays = officialAttendances.reduce((sum, row) => {
    ...
    if (row.competition.competitionType === "B") return sum + 0.5;
    return sum;
  }, 0);
  const officialAttendanceAClassDays = officialAttendances.filter(
    (row) => row.competition.competitionType === "A"
  ).length;
  const officialAttendanceBClassDays = officialAttendances.filter(
    (row) => row.competition.competitionType === "B"
  ).length;
