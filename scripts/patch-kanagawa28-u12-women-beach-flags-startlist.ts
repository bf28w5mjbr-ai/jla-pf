/**
 * U12女子ビーチフラッグス: 印刷スタートリストの HEAT/レーンに合わせる
 *
 *   pnpm exec tsx --tsconfig tsconfig.json --env-file=.env scripts/patch-kanagawa28-u12-women-beach-flags-startlist.ts --dry-run
 *   pnpm exec tsx --tsconfig tsconfig.json --env-file=.env scripts/patch-kanagawa28-u12-women-beach-flags-startlist.ts --execute
 */
...
const EVENT_ID = "cmnulws110007l404lyk85iz7";
...
function resolveParticipant(
  ...
  return byName.get(normalizeName(printName)) ?? null;
}
...
async function main() {
  ...
  console.log("\n[execute] U12女子ビーチフラッグス HEAT 割当を更新しました");
}
