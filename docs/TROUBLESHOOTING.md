...
## 公開スタートリストに結果が表示されない
- 大会のスタートリストが公開設定になっているか（主催の公開トグル、`startListPubliclyVisible`）。
- 当該ヒートで当日運用の「リザルト確定」が完了しているか。
- スナップショット上のヒート割と公式結果行の `heat` が一致しているか（不一致時はオーバーレイ非表示）。
- 他大会/他種目のエントリーIDを混在させると結果保存は 400 で失敗する。
